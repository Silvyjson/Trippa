...
const ActivitySection = ({ activityData }) => {
    ...
    const handleViewActivity = (index) => {
        ...
        let recentClickedActivities = JSON.parse(localStorage.getItem('recentClickedActivities')) || [];

        recentClickedActivities.unshift(activityData[index]);
        if (recentClickedActivities.length > 4) {
            recentClickedActivities.pop();
        }
        localStorage.setItem('recentClickedActivities', JSON.stringify(recentClickedActivities));
    };
